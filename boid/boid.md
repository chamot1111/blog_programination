...
The nice think when you looks at this kind of simulation is the simplicity. Each agent has only a few simple rules. The beauty comes from the interaction of all the agents. We see emergence of very complicated behaviour from the group that can resolve some problems. In this case, it's the mimic of bird fly.

Simulation
----------

Let's enter the Boid simulation in details. What we wants is to have a simulation of Boids as explain by Craig Reynold. The boid fly in a sphere.
...
These tree rules need to know all the boid in the field of view. So we get them only once in an array for each frame. We will see later how we capture the neighbors and at this moment we assume `neighbors` var contains an array with current boids in neighborhood.

### 1. Collision avoidance

Each Boids try to stay at a minimum distance of all his neighbors in his field of view. Each Boid under the minimum distance `kMinimumSeparationDistance` create a repulsion force linear proportional to distance (more closer, more powerful).
...
Boid tend to keep the same velocity as neighbors. We calculate the velocity average and add a fraction of the difference. If the boid was around at the same velocity as the neighbors, it will have almost any effect. The effect is proportional to the speed difference to the group average.
...
Each boid tend to stay at the gravity center of the neighbors in the field of view. It will give the effect of the organization of the flight.
